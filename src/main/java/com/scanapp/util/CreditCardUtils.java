...
public class CreditCardUtils {
    ...
    public static boolean isCreditCardNumber(String cardPan) {
        try {

            if (getCardID(cardPan) > -1) {
              //  System.out.println("This a " + getCardName(getCardID(cardPan)));
               /* System.out.println
                        ("The card number " + cardPan + " is "
                                + (validCC(cardPan) ? " good." : " bad."));*/
                return validCC(cardPan);
            } else {
                //System.out.println("This card is invalid or unsupported!");
            }

        } catch (Exception e) {
           // log.error("unable to validate card " + e.getLocalizedMessage(), e);
        }
        return false;
